const Game = {
    ...
    createCases: function() {
        ...
        for (let i = 1; i <= 100; i++) {
            ...
            for (let j = 0; j < 4; j++) {
                ...
                const nameIndex = (i * 3 + j * 5) % firstNames.length;
                const roleIndex = (i * 7 + j * 3) % roles.length;
                ...
                suspects.push({
                    ...
                    clueReaction: isGuilty
                        ? "Он начинает заметно нервничать и пытается сменить тему."
            ...
            cases.push({
                ...
                description:
                    ...
                    `Следователь обнаружил несколько противоречий, а показания свидетелей не совпадают.`,
                ...
                clues: [
                    `На месте найден фрагмент предмета с номером ${100 + i}.`,
                ...
                hint:
                    ...
                    `Особенно внимательно проверьте временной промежуток ` +
                    `и мотив, связанный с темой «${motive}».`
            });
        }
    ...
    updateHeader: function() {
        ...
        if (hints) {
            hints.textContent = this.state.hints;
        }
    },
    ...
    askQuestion: function(type = null) {
        ...
        this.addMessage(
            "det",
            question || this.questionText(questionType)
        );
        ...
        const increase = suspect?.guilty ? 13 : 6;
        this.addSuspicion(increase);

        setTimeout(() => {
            const answer = this.getSuspectAnswer(
                questionType,
                suspect
            );
    ...
    getSuspectAnswer: function(type, suspect) {
        if (!suspect) {
            return AIEngine.getAnswer(type, null);
        }
