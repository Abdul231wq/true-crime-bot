const AIEngine = {
  ...
  init() {
    this.state = {
      ...
      chapterHintMultiplier: this.readNumber("chapterHintMultiplier", this.defaults.chapterHintMultiplier),
      maxChapterHintMultiplier: this.readNumber("maxChapterHintMultiplier", this.defaults.maxChapterHintMultiplier),
      ...
      softHintCooldownMs: this.readNumber("softHintCooldownMs", this.defaults.softHintCooldownMs),
      lastSoftHintAt: this.readNumber("lastSoftHintAt", this.defaults.lastSoftHintAt),
      ...
      successfulQuestions: this.readNumber("successfulQuestions", this.defaults.successfulQuestions),
      correctHypotheses: this.readNumber("correctHypotheses", this.defaults.correctHypotheses),
      correctVerdicts: this.readNumber("correctVerdicts", this.defaults.correctVerdicts),
      hintHistory: this.readJSON("hintHistory", this.defaults.hintHistory),
      activeSuspect: localStorage.getItem(this.storageKey("activeSuspect")) || this.defaults.activeSuspect
    };
  ...
  readNumber(name, fallback) {
    const saved = localStorage.getItem(this.storageKey(name));
  ...
  readJSON(name, fallback) {
    const saved = localStorage.getItem(this.storageKey(name));
    if (saved === null) return Array.isArray(fallback) ? [...fallback] : fallback;
    try {
      const value = JSON.parse(saved);
      return Array.isArray(value) ? value : (Array.isArray(fallback) ? [...fallback] : fallback);
    } catch {
  ...
  normalizeState() {
    ...
    s.chapterProgress = this.clamp(Math.floor(Number(s.chapterProgress) || 0), 0, 100);
    ...
    s.cluesFound = Math.max(0, Math.floor(Number(s.cluesFound) || 0));
    s.successfulQuestions = Math.max(0, Math.floor(Number(s.successfulQuestions) || 0));
    s.correctHypotheses = Math.max(0, Math.floor(Number(s.correctHypotheses) || 0));
    s.correctVerdicts = Math.max(0, Math.floor(Number(s.correctVerdicts) || 0));
    if (!Array.isArray(s.hintHistory)) s.hintHistory = [];
    if (!s.activeSuspect) s.activeSuspect = this.defaults.activeSuspect;
  },
  ...
  setActiveSuspect(suspectId) {
    this.state.activeSuspect = suspectId;
    this.save();
    return suspectId;
  },

  getActiveSuspectName() {
    if (this.state.activeSuspect === "suspect-b") return "Марина Логинова";
    if (this.state.activeSuspect === "suspect-c") return "Игорь Белый";
    return "Алексей Морозов";
  },

  getChapterMultiplier() {
    const chapter = Math.max(1, this.state.currentChapter || 1);
    const multiplier = 1 + (chapter - 1) * this.state.chapterHintMultiplier;
    return Math.min(this.state.maxChapterHintMultiplier, multiplier);
  },
  ...
  getHintByStuckLevel() {
    ...
    if (level <= 0) return { type: "soft", text: "Проверь, у кого был мотив и доступ к месту преступления." };
    if (level === 1) return { type: "soft", text: "Сравни показания подозреваемых с временной линией события." };
    if (level === 2) return { type: "soft", text: "Одна из улик выглядит слабее остальных. Начни с неё." };
    if (level === 3) return { type: "hard", text: "Главный подозреваемый скрывает несоответствие в алиби." };
    return { type: "hard", text: "Ищи того, чьи слова не совпадают с записью звонка и следами на месте." };
  },

  spendInvestigationPoints(cost) {
    const safeCost = Math.max(0, Math.floor(Number(cost) || 0));
    if (this.state.investigationPoints < safeCost) return false;
  ...
  addClue() {
    ...
    this.state.chapterProgress = this.clamp(this.state.chapterProgress + 10, 0, 100);
    this.updateStuckLevelFromProgress();
    this.save();
    return { ok: true, message: "Найдена новая улика. Получено 2 очка расследования." };
  },
  ...
  successfulQuestion() {
    ...
    this.updateStuckLevelFromProgress();
    this.save();
    return { ok: true, message: "Допрос дал результат. Получено 3 очка расследования." };
  },

  correctHypothesis() {
    this.state.correctHypotheses += 1;
    this.state.investigationPoints += 4;
    this.state.chapterProgress = this.clamp(this.state.chapterProgress + 15, 0, 100);
    this.updateStuckLevelFromProgress();
    this.save();
    return { ok: true, message: "Гипотеза подтверждена. Получено 4 очка расследования." };
  },
  ...
  correctVerdict() {
    ...
    this.save();
    return { ok: true, message: "Верный вердикт. Получено 10 очков расследования." };
  },

  updateStuckLevelFromProgress() {
    const p = this.state.chapterProgress;
    if (p <= 0) this.state.stuckLevel = 0;
    else if (p < 25) this.state.stuckLevel = 1;
    else if (p < 50) this.state.stuckLevel = 2;
    else if (p < 75) this.state.stuckLevel = 3;
    else this.state.stuckLevel = 4;
  },

  setProgress(value) {
    const progress = this.clamp(Math.floor(Number(value) || 0), 0, 100);
  ...
  advanceChapter() {
    ...
    this.state.stuckLevel = 0;
    this.save();
    return { ok: true, chapter: this.state.currentChapter };
  },
  ...
  requestSoftHint() {
    ...
    this.state.stuckLevel = Math.min(4, this.state.stuckLevel + 1);
    this.state.hintHistory.unshift(`Мягкая (${cost} очков): ${hint.text}`);
    this.state.hintHistory = this.state.hintHistory.slice(0, 10);
    this.save();

    return { ok: true, type: "soft", hint, cost };
  },
  ...
  requestHardHint() {
    ...
    this.state.stuckLevel = Math.min(4, this.state.stuckLevel + 1);
    this.state.hintHistory.unshift(`Жёсткая (${cost} очков): ${hint.text}`);
    this.state.hintHistory = this.state.hintHistory.slice(0, 10);
    this.save();

    return { ok: true, type: "hard", hint, cost };
  },
